...
export class CategoriesPageComponent implements OnInit {
  ...
  toggleCategory(categoryId: string): void {
    if (this.expandedCategories.has(categoryId)) {
      ...
      this.buildSubcategoriesFromProducts(categoryId);
    }
  }

  buildSubcategoriesFromProducts(categoryId: string): void {

    if (this.categorySubcategories.has(categoryId)) return;
    ...
    for (const product of this.productsService.allProducts || []) {
      ...
      if (product.category?._id !== categoryId) continue;
    ...
    this.categorySubcategories.set(categoryId, subs);
